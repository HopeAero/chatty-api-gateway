...
export class AuthRestController {
  ...
  async register(@Body() credentials: RegistrationCredentials) {
    return await this.authService.register(credentials.username, credentials.password);
  }

  @UseGuards(AuthGuard('local'))
  @Post("login")
  async login(@Body() credentials: LoginCredentials) {
    const { jwt } = await this.authService.login(credentials.username, credentials.password);
    return jwt;
  }
}
